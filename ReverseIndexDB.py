class FileMetadata:
    def __init__(self, file_name):
        ...
        self.strings = []  # List to store tuples of (original_str, processed_str, line_num)

    def add_string(self, original_str, line_num):
        # Process the string before saving
        processed_str = self.process_text(original_str)
        self.strings.append((original_str, processed_str, line_num))  # Append the strings and line number as a tuple
...
class ReverseIndex:
    ...
    def add_word(self, word, file_name, original_str, line_num):
        ...
        if existing_metadata:
            # Add the original string and line number to the existing FileMetadata (it will be processed internally)
            existing_metadata.add_string(original_str, line_num)
        else:
            # Create new FileMetadata and add it to the list
            new_metadata = FileMetadata(file_name)
            new_metadata.add_string(original_str, line_num)
